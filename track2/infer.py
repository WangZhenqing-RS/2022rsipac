...
class OurDataset(D.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 百分比截断拉伸
        # if image_name in ["5.tif", "6.tif", "7.tif", "8.tif", "9.tif"]:
        ...
        # image_B = style_transfer(image_B, image_A)      # 掉点0.03 or more
        
        # # 直方图匹配
        # image_A = match_histograms(image_A, image_B, multichannel=True) # 掉点
        ...
        for i in range(output.shape[0]):
            ...
            for c in range(nc):
                if np.all(mask[label == c] == 0):
                    ...
                    # if score>=0.2:
                    #     score = (score-0.2)/(1-0.2)*(1-0.5)+0.5
                    # else:
                    #     score = (score-0.08)/(0.2-0.08)*(0.5-0.08)+0.08
                    if area>=10:
                        dts.append({
                            "segmentation": {
...
if __name__ == "__main__":
    ...
    test_json_path = "/test/instances_test.json"
    ...
    if not os.path.exists("/output/results"): os.makedirs("/output/results")
    with open("/output/results/test.segm.json", "w") as f1:
        json.dump(dts, f1)
    
    os.system("cd /output && zip -r results.zip results")
