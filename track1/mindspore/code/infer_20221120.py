...
from src.config import config_hrnetv2_18 as config
...
def parse_args():
    ...
    parser.add_argument("--checkpoint_path", type=str, default="./model/card0/hrnet48-40_1284.ckpt",
                        help="Storage path of checkpoint file on machine.")
    parser.add_argument("--checkpoint_path2", type=str, default="./model/card0/hrnet48-40_1284.ckpt",
                        help="Storage path of checkpoint file on machine.")
    parser.add_argument("--checkpoint_path3", type=str, default="./model/card0/hrnet48-40_1284.ckpt",
                        help="Storage path of checkpoint file on machine.")
...
class Hrnet_infer_TTA_two_model(nn.Cell):
    """
    Encapsulation class of centernet testing.
...
class Hrnet_infer(nn.Cell):
    ...
    Returns:
        Tensor, detection of images(bboxes, score, keypoints and category id of each objects)
    """
    def __init__(self, network, network2, network3, K=100, enable_nms_fp16=False):
        super(Hrnet_infer, self).__init__()
        self.network = network
        self.network2 = network2
    ...
    def construct(self, image):
        ...
        seg5, hm5, wh5, offset5 = self.network2(image_flip2)
        seg6, hm6, wh6, offset6 = self.network2(image_flip3)
        seg7, hm7, wh7, offset7 = self.network3(image)
        seg8, hm8, wh8, offset8 = self.network3(image_flip2)
        seg9, hm9, wh9, offset9 = self.network3(image_flip3)
        ...
        seg_outputs = (self.act(seg1) + 
                       ...
                       self.flip3(self.act(seg6)) +
                       self.act(seg7) + 
                       self.flip2(self.act(seg8)) + 
                       self.flip3(self.act(seg9))
                      ) / 9
        ...
        hm = (self.act(hm1) + 
              ...
              self.flip3(self.act(hm6)) +
              self.act(hm7) + 
              self.flip2(self.act(hm8)) + 
              self.flip3(self.act(hm9))
             ) / 9
...
if __name__ == '__main__':
    ...
    net3 = get_seg_model(config)
    param_dict3 = load_checkpoint(ckpt_file_name=config.checkpoint_path3)
    load_param_into_net(net3, param_dict3)
    net3.set_train(False)
    
    net_infer = Hrnet_infer(net, net2, net3, K=1000)
    net_infer.set_train(False)
    model = Model(net_infer)
    
    test_json_path = "data/fusai_release/testA/instances_test.json"
    test_image_folder = "data/fusai_release/testA/images"
