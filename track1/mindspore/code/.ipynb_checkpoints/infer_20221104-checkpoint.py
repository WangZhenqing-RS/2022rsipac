...
if __name__ == '__main__':
    ...
    for i in tqdm.tqdm(range(images_num)):
        ...
        try:
            ...
            image_path = image_path.replace("images","images_correct")
            print(image_path)
            image = io.imread(image_path)
            image = image[:,:,0:3]
            image = np.uint8(image)
        ...
        resize_scale_h = 5120 / image_shape[0]
        resize_scale_w = 5120 / image_shape[1]
        
        # image = cv2.resize(image,(3552, 5920))
        # resize_scale_h = 5920 / image_shape[0]
        # resize_scale_w = 3552 / image_shape[1]
        ...
        seg_outputs = cv2.resize(seg_outputs, (image_shape[1],image_shape[0]), interpolation=cv2.INTER_NEAREST)
        ...
        for det_output in det_outputs:
            ...
            annotations_test["annotations"].append({
                ...
                "iscrowd": 0,
                "image_id": image_id,
                "bbox": [int(_/resize_scale) for _,resize_scale in zip(det_output[:4],resize_scale_s)],
