...
class HighResolutionNet(nn.Cell):
    ...
    def __init__(self, config, **kwargs):
        ...
        inter_channels = last_inp_channels // 4

        self.resize_bilinear = nn.ResizeBilinear()

        # 分割部分
        self.fcn_head = nn.SequentialCell([
            nn.Conv2d(last_inp_channels, inter_channels, kernel_size=1, stride=1, pad_mode='pad', padding=0),
            batchNorm2d(inter_channels),
            nn.ReLU(),
            nn.Conv2d(inter_channels, config.seg_classes, kernel_size=1, stride=1, pad_mode='pad', padding=0)
        ])
        
        # 热力图预测部分
        self.cls_head = nn.SequentialCell([
            nn.Conv2d(last_inp_channels, inter_channels, kernel_size=3, pad_mode='pad', padding=1),
            batchNorm2d(inter_channels),
            nn.ReLU(),
            nn.Conv2d(inter_channels, config.det_classes, kernel_size=1, stride=1, pad_mode='pad', padding=0)
        ])
        
        # 宽高预测的部分
        self.wh_head = nn.SequentialCell([
            nn.Conv2d(last_inp_channels, inter_channels, kernel_size=3, pad_mode='pad', padding=1),
            batchNorm2d(inter_channels),
            nn.ReLU(),
            nn.Conv2d(inter_channels, 2, kernel_size=1, stride=1, pad_mode='pad', padding=0)
        ])
        

        # 中心点预测的部分
        self.reg_head = nn.SequentialCell([
            nn.Conv2d(last_inp_channels, inter_channels, kernel_size=3, pad_mode='pad', padding=1),
            batchNorm2d(inter_channels),
            nn.ReLU(),
            nn.Conv2d(inter_channels, 2, kernel_size=1, stride=1, pad_mode='pad', padding=0)
    ...
    def construct(self, x):
        ...
        # ops.Shape()返回输入Tensor的shape
        h, w = ops.Shape()(out1)[2:]
        # ops.Cast()转换输入Tensor的数据类型
        # x1 = ops.Cast()(out1, mindspore.dtype.float32)
        # x2 = ops.Cast()(self.resize_bilinear(out2, size=(h, w)), mindspore.dtype.float32)
        # x3 = ops.Cast()(self.resize_bilinear(out3, size=(h, w)), mindspore.dtype.float32)
        # x4 = ops.Cast()(self.resize_bilinear(out4, size=(h, w)), mindspore.dtype.float32)
        ...
        seg = self.fcn_head(feats)
        hm = self.cls_head(feats)
        wh = self.wh_head(feats)
        offset = self.reg_head(feats)
...
def get_seg_model(cfg, **kwargs):
    ...
    for name, cell in model.cells_and_names():
        if any(part in name for part in {'head'}):
